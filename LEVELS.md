...
Each entry in `spawn` is `{ type: string, count: number, ...options }`. `type` is either an `ASTEROID.sizes` key (`'large'`, `'medium'`, `'small'`, etc.), `'enemy'`, or `'cargo'`. Multiple entries are supported — e.g., a mix of large asteroids and an enemy on the same level.
...
- L28 introduces mine hazards (not yet implemented).
